            ...
            <div class="row">
                <div class="col-md-10">
                    <div class="row">
                            ...
                            <div class="form-group">
                                <select class="form-control" name="type_id" required>
                                    ...
                                    @foreach ($ar_type as $k=>$v)
                                        <option value="{{ $k }}" {{ $request->type_id == $k ? 'selected' : null }}>{{ $v }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="По логину" name="login" value="{{ $request->login }}" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="javascript:void(0)"
                                    ...
                                    data-val-type_id='{{ $i->type_id }}'
                                    data-val-login='{{ $i->login }}'
                                    data-val-name='{{ $i->name }}'
                                    data-val-contact='{{ $i->contact }}'
                                    data-val-address='{{ $i->address }}'
                                    data-val-id='{{ $i->id }}'>Изменить</a>
                                <a href="{{ action('Admin\System\UserController@getChangeActive', $i->id) }}"
...
@include('admin.system.user.__include.dialog_edit')
@endsection
